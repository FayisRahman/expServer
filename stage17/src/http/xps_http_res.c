...
xps_http_res_t *xps_http_res_create(xps_core_t *core, u_int code) {
  ...
  snprintf(http_res->response_line, sizeof(http_res->response_line), "HTTP/1.1 %d %s", code,
           reason_phrase);
...
xps_buffer_t *xps_http_res_serialize(xps_http_res_t *http_res) {
  ...
  logger(LOG_DEBUG, "xps_http_res_serialize()", "http response serialized succefully");

  // // Print response line and headers
  // logger(LOG_HTTP, "xps_http_res_serialize()", "%s", http_res->response_line);
  // for (int i = 0; i < http_res->headers.length; i++) {
  //   xps_keyval_t *header = http_res->headers.data[i];
  //   logger(LOG_HTTP, "xps_http_res_serialize()", "%s: %s", header->key, header->val);
  // }
...
void xps_http_res_set_body(xps_http_res_t *http_res, xps_buffer_t *buff) {
  ...
  sprintf(body_len_str, "%lu", buff->len);
  xps_http_set_header(&http_res->headers, "Content-Length", body_len_str);
}