...
int get_fd_type(int fd) {
  for (int i = 0; i < route_table_size; i++) {
    if (route_table[i][0] == fd)
      return CLIENT_SOCK_FD; // client
    if (route_table[i][1] == fd)
      return UPSTREAM_SOCK_FD; // upstream
  }
  return -1; // Not found.
}
...
void handle_client(int conn_sock_fd) {
  ...
  int upstream_sock_fd = find_pair(
    conn_sock_fd, UPSTREAM_SOCK_FD); // find the right upstream socket from the route table
...
void handle_upstream(int upstream_sock_fd) {
  ...
  int read_n = recv(upstream_sock_fd, buff, sizeof(buff),
                    0); /* read message from upstream to buffer using recv */
