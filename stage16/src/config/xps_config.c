...
void parse_listener(JSON_Object *listener_object,
                    xps_config_listener_t *listener);
void parse_route(JSON_Object *route_object, xps_config_route_t *route);
void parse_all_listeners(vec_void_t *_all_listeners,
                         xps_config_server_t *server);
...
xps_config_lookup_t *xps_config_lookup(xps_config_t *config,
  ...
  *error = E_FAIL;
  ...
  const char *h_keep_alive =
      xps_http_get_header(&(http_req->headers), "Connection");
  const char *h_accept_encoding =
      xps_http_get_header(&(http_req->headers), "Accept-Encoding");
  ...
  lookup->file_end = -1;
  lookup->upstream =
      route->upstreams.length > 0 ? route->upstreams.data[0] : NULL;
  /*till here |^*/
  ...
  if (lookup->type == REQ_FILE_SERVE) {
    ...
    if (!is_abs_path(
            resource_path)) { // we require abosulte path so we need to see if
    ...
    logger(LOG_DEBUG, "xps_config_file_lookup()", "requested file path: %s",
           lookup->file_path);
    *error = OK;
  }
  *error = OK;
...
void parse_route(JSON_Object *route_object, xps_config_route_t *route) {
  ...
  if (strcmp(route->type, "file_serve") &&
      strcmp(route->type, "reverse_proxy") && strcmp(route->type, "redirect")) {
  ...
  if (strcmp(route->type, "file_serve") == 0) {
    ...
    route->http_status_code =
        json_object_get_number(route_object, "http_status_code");
...
void parse_all_listeners(vec_void_t *_all_listeners,
  ...
  for (int i = 0; i < server->listeners.length; i++) {
    ...
    for (int j = 0; j < _all_listeners->length; j++) {
      ...
      if (strcmp(server_listener->host, curr->host) == 0 &&
          server_listener->port == curr->port) {
        listener_found = true;
        break;
      }
    }
