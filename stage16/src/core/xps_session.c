...
void upstream_source_handler(void *ptr) {
  ...
  logger(LOG_DEBUG, "upstream_source_handler()",
         "Called! Writing %zu bytes to upstream\n",
...
void session_process_request(xps_session_t *session) {
  ...
  xps_config_lookup_t *lookup = xps_config_lookup(
      session->core->config, session->http_req, session->client, &lookup_error);

  // TODO:MIGHT BE BETTER TO TIP THEM TO PROCESS THE ERROR (EG: E_FAIL,
  // E_NOTFOUND) AS WELL AND SEND APPROPRIATE RESPONSE BACK(EG:
  // HTTP_INTERNAL_SERVER_ERROR, HTTP_NOT_FOUND)
  if (lookup_error == E_FAIL) {
    logger(LOG_ERROR, "session_process_request()",
           "xps_config_lookup() failed");
    xps_http_res_t *http_res =
        xps_http_res_create(session->core, HTTP_INTERNAL_SERVER_ERROR);
    ...
    xps_http_res_t *http_res =
        xps_http_res_create(session->core, HTTP_NOT_FOUND);
    xps_buffer_t *http_res_buff = xps_http_res_serialize(http_res);
    set_to_client_buff(session, http_res_buff);
    xps_http_res_destroy(http_res);
    return;
  ...
  // TODO: NEED TO EXPLAIN SOME DETAILS ABOUT THE BELOW IN THE DOC CUZ I HAVE NO
  // IDEA WHAT HAPPENED BELOW
  if (lookup->type == REQ_FILE_SERVE) {
    ...
    if (lookup->file_path) {
      ...
      xps_file_t *file =
          xps_file_create(session->core, lookup->file_path, &error);
      ...
      // TODO: need to add this as well
      //  Directory without index file or file not found
      logger(LOG_WARNING, "session_process_request()",
             "No file to serve (directory without index)");
      xps_http_res_t *res = xps_http_res_create(session->core, HTTP_NOT_FOUND);
      xps_buffer_t *buff = xps_http_res_serialize(res);
      set_to_client_buff(session, buff);
      xps_http_res_destroy(res);
      return;
    ...
    if (session->upstream == NULL) {
      logger(LOG_ERROR, "session_process_request()",
             "failed to connect to upstream %s:%u", host, port);
      xps_http_res_t *http_res =
          xps_http_res_create(session->core, HTTP_BAD_GATEWAY);
      ...
      xps_pipe_create(session->core, DEFAULT_PIPE_BUFF_THRESH,
                      session->upstream->source, session->upstream_sink);
      xps_pipe_create(session->core, DEFAULT_PIPE_BUFF_THRESH,
                      session->upstream_source, session->upstream->sink);
