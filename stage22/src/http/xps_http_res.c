...
xps_http_res_t *xps_http_res_create(xps_core_t *core, u_int status_code) {
  ...
  snprintf(http_res->response_line, sizeof(http_res->response_line), "HTTP/1.1 %d %s", status_code,
           reason_phrase);
  ...
  if (code_start == 2)
    xps_metrics_set(core, M_RES_2XX, 1);
  else if (code_start == 3)
    xps_metrics_set(core, M_RES_3XX, 1);
  else if (code_start == 4)
    xps_metrics_set(core, M_RES_4XX, 1);
  else if (code_start == 5)
    xps_metrics_set(core, M_RES_5XX, 1);
