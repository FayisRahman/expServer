...
xps_connection_t *xps_connection_create(xps_core_t *core, u_int sock_fd) {
  ...
  xps_pipe_source_t *source =
    xps_pipe_source_create(connection, connection_source_handler, connection_source_close_handler);
  if (source == NULL) {
  ...
  xps_pipe_sink_t *sink =
    xps_pipe_sink_create(connection, connection_sink_handler, connection_sink_close_handler);
  if (sink == NULL) {
  ...
  int ret = xps_loop_attach(core->loop, sock_fd, EPOLLIN | EPOLLOUT | EPOLLET, connection,
